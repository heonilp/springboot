...
WAS: 주문 서버 -> ->HTTP API - 서버 to 서버  주문 정보 조회  -> DB - WAS: 결제 서버

웹 클라이언트 (자바스크립트)  ->HTTP API - 웹 클라이언트 to 서버->  WAS: 주문 서버 

앱 클라이언트 -> HTTP API - 앱 클라이언트 to 서버  -> WAS: 주문 서버 
(아이폰, 안드로이드, PC 앱 등등)
...
1. HTML 요청 /orders.html : HTML 내용X 자바스크립트 링크

2. 자바스크립트 요청 Javascript 클라이언트 로직 HTML 렌더링 코드

3. HTTP API - 데이터 요청 JSON {"상품명":"A", "가격:10000}

4. 웹브라우저 - 자바스크립트로 HTML 결과 렌더링
...
- 자바 웹 기술 역사 3. 최신 기술 - 스프링 웹 기술의 분화
...
- 자바 웹 기술 역사 4. 최신 기술 - 스프링 웹 플럭스(WebFlux)
...
- HTTP 요청 메시지 로그로 확인하기 -다음 설정을 추가하자.
